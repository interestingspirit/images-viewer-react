...
const ViewerNav = (props: ViewerNavProps): React.ReactElement => {
  ...
  const initMarginValue =
    activeIndex > 5 ? -(activeIndex - 5) * (navImgWidth + 10) : 0
  ...
  React.useEffect(() => {
    const ulContainer = ulRef.current || undefined
    const ulWidth = ulContainer.clientWidth
    const showNextButton =
      (navImgWidth + 10) * props.images.length + marginValue - 5 > ulWidth
    if (showNextButton) {
      setShowNext(true)
    } else {
      setShowNext(false)
  ...
  function move() {
    ...
    if (activeIndex < preActiveIndex.current) {
      ...
      if (
        activeIndex === 0 &&
        preActiveIndex.current === props.images.length - 1
      ) {
        // 从最后一张跳到第一张
        currentValue = 0
        ...
        currentValue =
          marginValue - (activeIndex - preActiveIndex.current) * itemOffset
      }
    } else if (activeIndex > preActiveIndex.current) {
      // 右移
      if (
        activeIndex === props.images.length - 1 &&
        preActiveIndex.current === 0
      ) {
        // 从第一张跳到最后一张
  ...
  return (
    <div className={`${props.prefixCls}-navbar divContainer`}>
      ...
      <div
        ...
        style={{
          margin: 'auto',
          width: (navImgWidth + 10) * props.images.length,
        }}
      >
        <ul
