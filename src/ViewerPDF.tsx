...
const ViewerPDF = (
  props: ViewerPDFProps,
  printRef: React.MutableRefObject<PrintRef>,
) => {
  ...
  const onRenderSuccess = ({ pageNumber }) => {
    ...
    if (pageNo === 0) {
      ...
      if (printing) {
        ...
        if (
          loadSuccessSize.current === nextSize ||
          loadSuccessSize.current === totalPages
        ) {
          // console.log(`${loadSuccessSize.current}页全部加载完成`);
          endLoading()
        }
  ...
  translateX(${props.left !== null ? props.left + 'px' : 'aoto'}) translateY(${
      ...
      rotate(${props.rotate}deg) scaleX(${props.scaleX}) scaleY(${
        props.scaleY
      })`,
    }
